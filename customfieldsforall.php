...
class plgVmCustomCustomfieldsforall extends vmCustomPlugin
{
	...
	/**
     * Отображает настраиваемое поле в представлении продукта серверной части
	...
	/**
     * Создает внутренний вывод плагина внутри продукта BE - administrator
...
//	    die(__FILE__ .' '. __LINE__ );
	...
	/**
	 * Получите настраиваемые поля продукта
	...
	/**
     * Сохранение  настраиваемые поля для конкретного продукта BE - administrator
...
//        die(__FILE__ .' '. __LINE__ );
	...
	/**
     * Запускается плагином stockableCustomfields при сохранении продукта BE - administrator
...
//	    die(__FILE__ .' '. __LINE__ );
	...
	/**
     * Сохраните все значения для всех настраиваемых полей
     * Эта функция должна запускаться только один раз для каждого продукта.
     *
...
//	    die(__FILE__ .' '. __LINE__ );
	...
	/**
     * Переопределите эту функцию, так как мы не хотим, чтобы  VM хранила данные подключаемого модуля. BE
	 * Override this function as we do not want VM to store the plug-in data
	 * @see vmCustomPlugin::storePluginInternalDataProduct()
     * @since 3.9
	 */
	protected function storePluginInternalDataProduct (&$values, $primaryKey = 0, $product_id = 0): bool
    {
//	    die(__FILE__ .' '. __LINE__ );
    ...
    /**
     * Отображение вариантов корзины / вариантов без корзины Пользовательские поля - VM3
     *
	...
	/**
     * Вычисляет цену продукта, применяя определенные / выбранные значения настраиваемого поля - VM3
     ...
     * @since 3.9
	 */
	public function plgVmPrepareCartProduct( &$product, &$customfield , $selected, float &$modificatorSum)
	{
	...
	/**
	 * Печатает поля статическим образом - выбор невозможен
	 ...
	 * @author	Sakis Terz
	 */
	protected function printStaticFields($productCustom,$values,&$html,$inline_css=false,$display_color_label=true)
    {
        $document = JFactory::getDocument();
        $document->addStyleSheet(JURI::root(true) . '/plugins/vmcustom/customfieldsforall/assets/css/customsforall_fe.css');
        $separator = '';
    ...
    /**
     * Trigger - просмотр товара в корзине
	...
	function plgVmOnViewCartVM3(&$product, &$productCustom, &$html,$inline_css=false,$display_color_label=true)
	{
        ...
        /*echo'<pre>';print_r( $product->customProductData );echo'</pre>'.__FILE__.' '.__LINE__ . PHP_EOL;
    ...
    /**
     * Trigger при удалении товара BE
     *
	...
	function plgVmOnDeleteProduct($virtuemart_product_id, $ok)
	{
        ...
        /**
         * @see /plugins/vmcustom/customfieldsforall/helpers/Customfield.php
         */
    ...
    /**
     * Trigger - при редактировании любого кастомарного поля BE - administrator
     * @param $virtuemart_custom_id
     * @param $customPlugin
     *
     * @since  3.9
     * @auhtor Gartes | sad.net79@gmail.com | Skype : agroparknew | Telegram : @gartes
     * @date   11.08.2021 19:46
     *
     */
...
//	    die(__FILE__ .' '. __LINE__ );
	...
	/**
     * Корзина товаров покупателя FE - VM3
     * Также используется для создания счета-фактуры
     *
	...
	/**
     * Хук для создания пользовательских фильтров из плагина
     *
